...
| 场景 | 吞吐 |
|---|---|
| 单流解码（贪心，KV 缓存 + 硬件 FP4 解码） | ~7 tok/s |
| 聚合批处理（bf16 张量核 GEMM） | batch 32 → 91，batch 64 → 143，batch 256 → ~216 tok/s |

单流受显存带宽约束（每 token 读 ~22GB 权重 ÷ 273 GB/s ≈ 12.4 tok/s 上限）；聚合靠一次
权重扫描服务多行（batch）来提高吞吐。张量核 GEMM 的进一步流水线优化（cp.async 双缓冲）仍在进行。
